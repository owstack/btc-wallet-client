...
function _checkNetwork(network) {
  if (!_.includes([Constants.LIVENET, Constants.TESTNET], network)) throw new Error('Invalid network');
};
...
Credentials.create = function(network) {
  ...
  x.network = network;
  x.xPrivKey = (new Btccore.HDPrivateKey(network)).toString();
...
Credentials.fromExtendedPublicKey = function(xPubKey, source, entropySourceHex, account, derivationStrategy, opts) {
  ...
  var x = new Credentials();
  x.xPubKey = xPubKey;
  x.entropySource = Btccore.crypto.Hash.sha256sha256(entropyBuffer).toString('hex');
  x.account = account;
  x.derivationStrategy = derivationStrategy;
...
Credentials._getNetworkFromExtendedKey = function(xKey) {
  ...
  return xKey.charAt(0) == 't' ? Constants.TESTNET : Constants.LIVENET;
};
...
Credentials.prototype._hashFromEntropy = function(prefix, length) {
  ...
  var b2 = Btccore.crypto.Hash.sha256hmac(b, new Buffer(prefix));
...
Credentials.prototype._expand = function() {
  ...
  if (this.xPrivKey) {
    var xPrivKey = new Btccore.HDPrivateKey.fromString(this.xPrivKey);
  ...
  if (this.entropySourcePath) {
    ...
    this.entropySource = Btccore.crypto.Hash.sha256sha256(seed).toString('hex');
  }

  if (this.entropySource) {
    // request keys from entropy (hw wallets)
    var seed = this._hashFromEntropy('reqPrivKey', 32);
    var privKey = new Btccore.PrivateKey(seed.toString('hex'), network);
    ...
    this.entropySource = Btccore.crypto.Hash.sha256(requestDerivation.privateKey.toBuffer()).toString('hex');
  }
...
Credentials.prototype.getBaseAddressDerivationPath = function() {
  ...
  var coin = (this.network == Constants.LIVENET ? "0" : "1");
...
Credentials.prototype.getDerivedXPrivKey = function(password) {
  ...
  var xPrivKey = new Btccore.HDPrivateKey(this.getKeys(password).xPrivKey, this.network);
  var deriveFn = !!this.compliantDerivation ? _.bind(xPrivKey.deriveChild, xPrivKey) : _.bind(xPrivKey.deriveNonCompliantChild, xPrivKey);
...
Credentials.fromOldCopayWallet = function(w) {
  function walletPrivKeyFromOldCopayWallet(w) {
    // IN BTCWS, the master Pub Keys are not sent to the server, 
    ...
    var privKey = new Btccore.PrivateKey.fromBuffer(Btccore.crypto.Hash.sha256(seedBuf));
  ...
  var pkr = _.map(w.publicKeyRing.copayersExtPubKeys, function(xPubStr) {
    ...
    if (isMe) {
      ...
      requestDerivation = (new Btccore.HDPrivateKey(credentials.xPrivKey))
        .deriveChild(path).hdPublicKey;
    } else {
      // this 
      var path = Constants.PATHS.REQUEST_KEY_AUTH;
      requestDerivation = (new Btccore.HDPublicKey(xPubStr)).deriveChild(path);
    }

    // Grab Copayer Name
    var hd = new Btccore.HDPublicKey(xPubStr).deriveChild('m/2147483646/0/0');
