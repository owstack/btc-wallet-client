...
PayPro.get = function(opts, cb) {
  ...
  opts.headers = opts.headers || {
    'Accept': BtccorePayPro.PAYMENT_REQUEST_CONTENT_TYPE,
  ...
  http(opts, function(err, dataBuffer) {
    ...
    try {
      var body = BtccorePayPro.PaymentRequest.decode(dataBuffer);
      request = (new BtccorePayPro()).makePaymentRequest(body);
    ...
    var decodedDetails = BtccorePayPro.PaymentDetails.decode(serializedDetails);
    var pd = new BtccorePayPro();
    ...
    var network = pd.get('network') == 'test' ? Constants.TESTNET : Constants.LIVENET;
    ...
    var addr = new Btccore.Address.fromScript(new Btccore.Script(scriptBuf), network);
...
PayPro._getPayProRefundOutputs = function(addrStr, amount) {
  ...
  var output = new BtccorePayPro.Output();
  var addr = new Btccore.Address(addrStr);

  var s;
  if (addr.isPayToPublicKeyHash()) {
    s = Btccore.Script.buildPublicKeyHashOut(addr);
  } else if (addr.isPayToScriptHash()) {
    s = Btccore.Script.buildScriptHashOut(addr);
  } else {
...
PayPro.send = function(opts, cb) {
  ...
  opts.method = 'POST';
  opts.headers = opts.headers || {
    'Accept': BtccorePayPro.PAYMENT_ACK_CONTENT_TYPE,
    'Content-Type': BtccorePayPro.PAYMENT_CONTENT_TYPE,
    // 'Content-Type': 'application/octet-stream',
  };
  opts.body = payment;
  ...
  http(opts, function(err, rawData) {
    ...
    if (rawData) {
      try {
        var data = BtccorePayPro.PaymentACK.decode(rawData);
        var pp = new BtccorePayPro();
