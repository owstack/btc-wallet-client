...
PayPro.get = function(opts, cb) {
  ...
  opts.headers = opts.headers || {
    'Accept': btcPayPro.PAYMENT_REQUEST_CONTENT_TYPE,
  ...
  http(opts, function(err, dataBuffer) {
    ...
    try {
      var body = btcPayPro.PaymentRequest.decode(dataBuffer);
      request = (new btcPayPro()).makePaymentRequest(body);
    ...
    var decodedDetails = btcPayPro.PaymentDetails.decode(serializedDetails);
    var pd = new btcPayPro();
    ...
    var addr = new btcLib.Address.fromScript(new btcLib.Script(scriptBuf), network);
...
PayPro._getPayProRefundOutputs = function(addrStr, amount) {
  ...
  var output = new btcPayPro.Output();
  var addr = new btcLib.Address(addrStr);

  var s;
  if (addr.isPayToPublicKeyHash()) {
    s = btcLib.Script.buildPublicKeyHashOut(addr);
  } else if (addr.isPayToScriptHash()) {
    s = btcLib.Script.buildScriptHashOut(addr);
  } else {
...
PayPro.send = function(opts, cb) {
  ...
  opts.method = 'POST';
  opts.headers = opts.headers || {
    'Accept': btcPayPro.PAYMENT_ACK_CONTENT_TYPE,
    'Content-Type': btcPayPro.PAYMENT_CONTENT_TYPE,
    // 'Content-Type': 'application/octet-stream',
  };
  opts.body = payment;
  ...
  http(opts, function(err, rawData) {
    ...
    if (rawData) {
      try {
        var data = btcPayPro.PaymentACK.decode(rawData);
        var pp = new btcPayPro();
