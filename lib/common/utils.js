...
var crypto = Btccore.crypto;
var encoding = Btccore.encoding;
...
Utils.parseNetFromNetworkURI = function(networkURI) {
  networkURI = networkURI || Constants.LIVENET;
  if (!_.includes([Constants.TESTNET, Constants.LIVENET], networkURI)) return cb(new Error('Invalid network'));
...
Utils.hashMessage = function(text) {
  ...
  ret = new Btccore.encoding.BufferReader(ret).readReverse();
  return ret;
};
...
Utils.privateKeyToAESKey = function(privKey) {
  ...
  $.checkArgument(Btccore.PrivateKey.isValid(privKey), 'The private key received is invalid');
  var pk = Btccore.PrivateKey.fromString(privKey);
  return Btccore.crypto.Hash.sha256(pk.toBuffer()).slice(0, 16).toString('base64');
};
...
Utils.deriveAddress = function(scriptType, publicKeyRing, path, m, network) {
  ...
  var publicKeys = _.map(publicKeyRing, function(item) {
    var xpub = new Btccore.HDPublicKey(item.xPubKey);
  ...
  switch (scriptType) {
    case Constants.SCRIPT_TYPES.P2SH:
      btccoreAddress = Address.createMultisig(publicKeys, m, network);
      break;
    case Constants.SCRIPT_TYPES.P2PKH:
      $.checkState(_.isArray(publicKeys) && publicKeys.length == 1);
      btccoreAddress = Address.fromPublicKey(publicKeys[0], network);
      break;
  }

  return {
    address: btccoreAddress.toString(),
    path: path,
...
Utils.signRequestPubKey = function(requestPubKey, xPrivKey) {
  var priv = new Btccore.HDPrivateKey(xPrivKey).deriveChild(Constants.PATHS.REQUEST_KEY_AUTH).privateKey;
  return Utils.signMessage(requestPubKey, priv);
};

Utils.verifyRequestPubKey = function(requestPubKey, signature, xPubKey) {
  var pub = (new Btccore.HDPublicKey(xPubKey)).deriveChild(Constants.PATHS.REQUEST_KEY_AUTH).publicKey;
...
Utils.buildTx = function(txp) {
  ...
  if (txp.toAddress && txp.amount && !txp.outputs) {
    ...
    _.each(txp.outputs, function(o) {
      ...
      if (o.script) {
        t.addOutput(new Btccore.Transaction.Output({
          script: o.script,
          satoshis: o.amount
        }));
      } else {
  ...
  // Validate inputs vs outputs independently of Btccore
