...
var BASE_URL = 'http://localhost:3232/btcws/api';
...
API.prototype.seedFromRandom = function(opts) {
  ...
  opts = opts || {};
  this.credentials = Credentials.create(opts.network || Constants.LIVENET);
};
...
API.prototype.validateKeyDerivation = function(opts, cb) {
  ...
  function testHardcodedKeys() {
    ...
    var xpub = Btccore.HDPublicKey.fromString('xpub6BosfCnifzxcFwrSzQiqu2DBVTshkCXacvNsWGYJVVhhawA7d4R5WSWGFNbi8Aw6ZRc1brxMyWMzG3DSSSSoekkudhUd9yLb6qx39T9nMdj');
  ...
  function testLiveKeys() {
    ...
    if (!xpriv) {
      xpriv = new Btccore.HDPrivateKey(c.xPrivKey);
    }
    xpriv = xpriv.deriveChild(c.getBaseAddressDerivationPath());
    var xpub = new Btccore.HDPublicKey(c.xPubKey);
...
API.prototype.seedFromRandomWithMnemonic = function(opts) {
  ...
  opts = opts || {};
  this.credentials = Credentials.createWithMnemonic(opts.network || Constants.LIVENET, opts.passphrase, opts.language || 'en', opts.account || 0);
};
...
API.prototype.seedFromMnemonic = function(words, opts) {
  ...
  opts = opts || {};
  this.credentials = Credentials.fromMnemonic(opts.network || Constants.LIVENET, words, opts.passphrase, opts.account || 0, opts.derivationStrategy || Constants.DERIVATION_STRATEGIES.BIP44, opts);
};
...
API.prototype._import = function(cb) {
  ...
  // First option, grab wallet info from BTCWS.
...
API.prototype.importFromMnemonic = function(words, opts, cb) {
  ...
  function derive(nonCompliantDerivation) {
    return Credentials.fromMnemonic(opts.network || Constants.LIVENET, words, opts.passphrase, opts.account || 0, opts.derivationStrategy || Constants.DERIVATION_STRATEGIES.BIP44, {
...
API.prototype.decryptBIP38PrivateKey = function(encryptedPrivateKeyBase58, passphrase, opts, cb) {
  ...
  var privateKey = new Btccore.PrivateKey(privateKeyWif);
  var address = privateKey.publicKey.toAddress().toString();
  var addrBuff = new Buffer(address, 'ascii');
  var actualChecksum = Btccore.crypto.Hash.sha256sha256(addrBuff).toString('hex').substring(0, 8);
  var expectedChecksum = Btccore.encoding.Base58Check.decode(encryptedPrivateKeyBase58).toString('hex').substring(6, 14);
...
API.prototype.getBalanceFromPrivateKey = function(privateKey, cb) {
  ...
  var privateKey = new Btccore.PrivateKey(privateKey);
...
API.prototype.buildTxFromPrivateKey = function(privateKey, destinationAddress, opts, cb) {
  ...
  async.waterfall([
    ...
    function(utxos, next) {
      ...
      var tx;
      try {
        var toAddress = Btccore.Address.fromString(destinationAddress);

        tx = new Btccore.Transaction()
          .from(utxos)
          .to(toAddress, amount)
          .fee(fee)
          .sign(privateKey);
...
API.prototype._getHeaders = function(method, url, args) {
  var headers = {
    'x-client-version': 'btcwc-' + Package.version,
  };
...
API._buildSecret = function(walletId, walletPrivKey, network) {
  if (_.isString(walletPrivKey)) {
    walletPrivKey = Btccore.PrivateKey.fromString(walletPrivKey);
  }
  var widHex = new Buffer(walletId.replace(/-/g, ''), 'hex');
  var widBase58 = new Btccore.encoding.Base58(widHex).toString();
  return _.padRight(widBase58, 22, '0') + walletPrivKey.toWIF() + (network == Constants.TESTNET ? 'T' : 'L');
};
...
API.parseSecret = function(secret) {
  ...
  try {
    ...
    var widHex = Btccore.encoding.Base58.decode(widBase58).toString('hex');
    var walletId = split(widHex, [8, 12, 16, 20]).join('-');

    var walletPrivKey = Btccore.PrivateKey.fromString(secretSplit[1]);
    var networkChar = secretSplit[2];

    return {
      walletId: walletId,
      walletPrivKey: walletPrivKey,
      network: networkChar == 'T' ? Constants.TESTNET : Constants.LIVENET,
    };
  } catch (ex) {
...
API.signTxp = function(txp, derivedXPrivKey) {
  ...
  var xpriv = new Btccore.HDPrivateKey(derivedXPrivKey);
...
API.prototype._addSignaturesToBtccoreTx = function(txp, t, signatures, xpub) {
  if (signatures.length != txp.inputs.length)
    throw new Error('Number of signatures does not match number of inputs');

  var i = 0,
    x = new Btccore.HDPublicKey(xpub);

  _.each(signatures, function(signatureHex) {
    var input = txp.inputs[i];
    try {
      var signature = Btccore.crypto.Signature.fromString(signatureHex);
      var pub = x.deriveChild(txp.inputPaths[i]).publicKey;
      var s = {
        inputIndex: i,
        signature: signature,
        sigtype: Btccore.crypto.Signature.SIGHASH_ALL,
        publicKey: pub,
      };
...
API.prototype._applyAllSignatures = function(txp, t) {
  ...
  _.each(sigs, function(x) {
    self._addSignaturesToBtccoreTx(txp, t, x.signatures, x.xpub);
  });
};
...
API.prototype.getFeeLevels = function(network, cb) {
  ...
  $.checkArgument(network || _.includes([Constants.LIVENET, Constants.TESTNET], network));
  network = Utils.parseNetFromNetworkURI(network || Constants.LIVENET);

  self._doGetRequest('/v3/feelevels/' + network, function(err, result) {
    if (err) return cb(err);
...
API.prototype.createWallet = function(walletName, copayerName, m, n, opts, cb) {
  ...
  if (opts.network != self.credentials.network) {
    return cb(new Error('Existing keys were created for a different network'));
  }

  var walletPrivKey = opts.walletPrivKey || new Btccore.PrivateKey();
  ...
  var args = {
    ...
    m: m,
    n: n,
    pubKey: (new Btccore.PrivateKey(walletPrivKey)).toPublicKey().toString(),
    network: Utils.parseNetFromNetworkURI(opts.network),
...
API.prototype.recreateWallet = function(cb) {
  ...
  this.getStatus({
    ...
    var c = self.credentials;
    var walletPrivKey = Btccore.PrivateKey.fromString(c.walletPrivKey);
    ...
    var args = {
      ...
      network: Utils.parseNetFromNetworkURI(c.network),
      id: walletId,
...
API.prototype.createTxProposal = function(opts, cb) {
  ...
  args.network = Utils.parseNetFromNetworkURI(args.network);
...
API.signTxProposalFromAirGapped = function(key, txp, unencryptedPkr, m, n, opts) {
  ...
  var newClient = new API({
    baseUrl: 'https://btcws.example.com/btcws/api'
  });

  if (key.slice(0, 4) === 'xprv' || key.slice(0, 4) === 'tprv') {
    if (key.slice(0, 4) === 'xprv' && txp.network == Constants.TESTNET) throw new Error("testnet HD keys must start with tprv");
    if (key.slice(0, 4) === 'tprv' && txp.network == Constants.LIVENET) throw new Error("livenet HD keys must start with xprv");
...
API.prototype.broadcastRawTx = function(opts, cb) {
  ...
  opts = opts || {};
  opts.network = Utils.parseNetFromNetworkURI(opts.network);
...
API.prototype.addAccess = function(opts, cb) {
  ...
  var reqPrivKey = new Btccore.PrivateKey(opts.generateNewKey ? null : this.credentials.requestPrivKey);
  var requestPubKey = reqPrivKey.toPublicKey().toString();

  var xPriv = new Btccore.HDPrivateKey(this.credentials.xPrivKey)
