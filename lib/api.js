...
API.prototype.validateKeyDerivation = function(opts, cb) {
  ...
  function testHardcodedKeys() {
    ...
    var xpub = btcLib.HDPublicKey.fromString('xpub6BosfCnifzxcFwrSzQiqu2DBVTshkCXacvNsWGYJVVhhawA7d4R5WSWGFNbi8Aw6ZRc1brxMyWMzG3DSSSSoekkudhUd9yLb6qx39T9nMdj');
  ...
  function testLiveKeys() {
    ...
    if (!xpriv) {
      xpriv = new btcLib.HDPrivateKey(c.xPrivKey);
    }
    xpriv = xpriv.deriveChild(c.getBaseAddressDerivationPath());
    var xpub = new btcLib.HDPublicKey(c.xPubKey);
...
API.prototype.getBalanceFromPrivateKey = function(privateKey, cb) {
  ...
  var privateKey = new btcLib.PrivateKey(privateKey);
  var address = privateKey.publicKey.toAddress();
  self.getUtxos({
    addresses: address.toString(),
  }, function(err, utxos) {
    if (err) return cb(err);
    return cb(null, _.sumBy(utxos, 'satoshis'), address.toString());
  });
};
...
API.prototype.buildTxFromPrivateKey = function(privateKey, destinationAddress, opts, cb) {
  ...
  async.waterfall([
    ...
    function(utxos, next) {
      ...
      var tx;
      try {
        var toAddress = btcLib.Address.fromString(destinationAddress);

        tx = new btcLib.Transaction()
          .from(utxos)
          .to(toAddress, amount)
          .fee(fee)
          .sign(privateKey);
...
API._buildSecret = function(walletId, walletPrivKey, network) {
  if (_.isString(walletPrivKey)) {
    walletPrivKey = btcLib.PrivateKey.fromString(walletPrivKey);
  }
  var widHex = new Buffer(walletId.replace(/-/g, ''), 'hex');
  var widBase58 = new btcLib.encoding.Base58(widHex).toString();
  return _.padEnd(widBase58, 22, '0') + walletPrivKey.toWIF() + (network == Constants.TESTNET ? 'T' : 'L') + Constants.CURRENCY_CHAR;
};
...
API.parseSecret = function(secret) {
  ...
  try {
    ...
    var widHex = btcLib.encoding.Base58.decode(widBase58).toString('hex');
    var walletId = split(widHex, [8, 12, 16, 20]).join('-');

    var walletPrivKey = btcLib.PrivateKey.fromString(secretSplit[1]);
    var networkChar = secretSplit[2];
    var currencyChar = secretSplit[3];

    if (currencyChar != Constants.CURRENCY_CHAR) {
      // Secret is not for this network currency
      throw false;
    }
...
API.signTxp = function(txp, derivedXPrivKey) {
  ...
  var xpriv = new btcLib.HDPrivateKey(derivedXPrivKey);
...
API.prototype._addSignaturesToBtcTx = function(txp, t, signatures, xpub) {
  if (signatures.length != txp.inputs.length)
    throw new Error('Number of signatures does not match number of inputs');

  var i = 0,
    x = new btcLib.HDPublicKey(xpub);

  _.each(signatures, function(signatureHex) {
    var input = txp.inputs[i];
    try {
      var signature = btcLib.crypto.Signature.fromString(signatureHex);
      var pub = x.deriveChild(txp.inputPaths[i]).publicKey;
      var s = {
        inputIndex: i,
        signature: signature,
        sigtype: btcLib.crypto.Signature.SIGHASH_ALL,
        publicKey: pub,
      };
...
API.prototype._applyAllSignatures = function(txp, t) {
  ...
  _.each(sigs, function(x) {
    self._addSignaturesToBtcTx(txp, t, x.signatures, x.xpub);
  });
};
...
API.prototype.getFeeLevels = function(network, cb) {
  ...
  network = network || Constants.LIVENET;
...
API.prototype.createWallet = function(walletName, copayerName, m, n, opts, cb) {
  ...
  var walletPrivKey = opts.walletPrivKey || new btcLib.PrivateKey();
  ...
  var args = {
    ...
    m: m,
    n: n,
    pubKey: (new btcLib.PrivateKey(walletPrivKey)).toPublicKey().toString(),
    network: network,
...
API.prototype.recreateWallet = function(cb) {
  ...
  this.getStatus({
    ...
    var c = self.credentials;
    var walletPrivKey = btcLib.PrivateKey.fromString(c.walletPrivKey);
...
API.prototype.addAccess = function(opts, cb) {
  ...
  var reqPrivKey = new btcLib.PrivateKey(opts.generateNewKey ? null : this.credentials.requestPrivKey);
  var requestPubKey = reqPrivKey.toPublicKey().toString();

  var xPriv = new btcLib.HDPrivateKey(this.credentials.xPrivKey)
